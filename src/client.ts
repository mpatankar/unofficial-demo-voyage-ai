...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['VOYAGE_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['VOYAGE_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Voyage API.
 */
export class Voyage {
  apiKey: string;
   ...
   * API Client for interfacing with the Voyage API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['VOYAGE_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['VOYAGE_BASE_URL'] ?? https://api.voyageai.com/v1] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('VOYAGE_BASE_URL'),
    apiKey = readEnv('VOYAGE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.VoyageError(
        "The VOYAGE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Voyage client with an apiKey option, like new Voyage({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Voyage.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('VOYAGE_LOG'), "process.env['VOYAGE_LOG']", this) ??
      defaultLogLevel;
  ...
  static VoyageError = Errors.VoyageError;
